...
test("cleanBalance filters correctly", () => {
	const raw = [
		{ asset: "USDT", balance: "100.0", available: "100.0", locked: "0.0" },
	];
	expect(cleanBalance(raw)).toEqual([
		{ asset: "USDT", balance: "100.0", available: "100.0" },
	]);
});
...
test("cleanBalance handles edge cases", () => {
	...
	expect(cleanBalance([{}] as any)).toEqual([
		{ asset: undefined, balance: undefined, available: undefined },
	]);
});
...
test("cleanPositions filters correctly", () => {
	...
	expect(cleanPositions(raw)).toEqual([
		{
			symbol: "BTCUSDT",
			entryPrice: "50000",
			markPrice: "51000",
...
test("cleanPositions handles edge cases", () => {
	...
	expect(cleanPositions([{}] as any)).toEqual([
		{
			symbol: undefined,
...
test("cleanOpenOrders filters correctly", () => {
	...
	expect(cleanOpenOrders(raw)).toEqual([
		{
			symbol: "BTCUSDT",
			price: "49000",
			quantity: "0.1",
...
test("cleanTradeHistory filters correctly", () => {
	...
	expect(cleanTradeHistory(raw)).toEqual([
		{
			symbol: "BTCUSDT",
			price: "50000",
			quantity: "0.1",
...
test("cleanTradeHistory handles edge cases", () => {
	...
	expect(cleanTradeHistory([{}] as any)).toEqual([
		{
			symbol: undefined,
			price: undefined,
			quantity: undefined,
...
test("cleanPnL filters correctly", () => {
	const raw = {
		totalUnrealizedProfit: "10.5",
		totalEquity: "110.5",
		other: "ignore",
	};
	expect(cleanPnL(raw)).toEqual({
		totalUnrealizedProfit: "10.5",
		totalEquity: "110.5",
	});
});
...
test("cleanPnL handles edge cases", () => {
	expect(cleanPnL(null as any)).toEqual({
		totalUnrealizedProfit: "0",
		totalEquity: "0",
	});
	expect(cleanPnL(undefined as any)).toEqual({
		totalUnrealizedProfit: "0",
		totalEquity: "0",
	});
	expect(cleanPnL({} as any)).toEqual({
